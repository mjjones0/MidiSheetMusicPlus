...
public class NoteColorDialog {
    ...
    /** Get/Set the colors used for each note.  There are 12 colors
     * in the array.
     */
    public Color[] Colors {
        get { return colors; }
    ...
    void SetColors(Color[] newcolors) {
        if (newcolors == null || newcolors.Length != 12) {
            return;
        }
        for (int i = 0; i < newcolors.Length; i++) {
            colors[i] = buttons[i].BackColor = buttons[i].ForeColor = newcolors[i];
        }
    }

    /** Get/Set the shade color selected */
    public Color ShadeColor {
        get { return shadeColor; }
        set { shadeColor = shadeButton.BackColor = shadeButton.ForeColor = value; }
    }

    /** Get/Set the shade2 color selected */
    public Color Shade2Color {
        get { return shade2Color; }
        set { shade2Color = shade2Button.BackColor = shade2Button.ForeColor = value; }
    }
